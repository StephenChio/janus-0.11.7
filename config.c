...
/* Helper to recursively process a libconfig setting 递归处理libconfig设置的助手 */
...
janus_config *janus_config_parse(const char *config_file) {
	...
	/* Create configuration instance 创建一个配置实例 */
	janus_config *jc = g_malloc0(sizeof(janus_config));
	/* 赋值配置实例的名称 */
	jc->name = g_strdup(filename);
	/* Is this a libconfig or INI config file?  判断配置文件属于libconfig还是ini config*/
	jc->is_jcfg = is_jcfg_config(jc->name);
	if(jc->is_jcfg) {
		/* Parse with libconfig and not manually 使用libconfig库手动解析libconfig */
		config_t config;
		config_init(&config);
